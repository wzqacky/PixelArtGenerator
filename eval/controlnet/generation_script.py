...
def generate_model_images(model_type: str, pipeline: DiffusionPipeline, config: Dict[str, Any]) -> None:
    ...
    num_control_per_prompt = config["num_control_per_prompt"]
    num_images_per_control = config["num_images_per_control"]
    num_images_per_prompt = num_control_per_prompt * num_images_per_control
    if model_type == "base":
        ...
        output_folder = config[f"{model_type}_output_folder"]
        seed_offset += 2000
    control_paths = config["control_paths"]
    if len(prompts) * num_control_per_prompt != len(control_paths):
        print("Error: Number of prompts and control images do not match.")
        sys.exit(1)
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        for seed_idx, seed in enumerate(seeds):
            ...
            control_path = control_paths[(seed_idx+1)*prompt_idx]
            control_image = load_image(control_path)\
                .resize((config["image_resolution"][0], config["image_resolution"][1]))
            with torch.no_grad():
                result = pipeline(
                    prompt=prompt,
                    image=control_image,
            ...
            image_filename = f"{model_type}_condition_{prompt_idx:02d}_control_{os.path.basename(control_path)}_seed_{seed}.png"
...
def main():
    ...
    # Expand control_paths by num_images_per_control
    control_paths = []
    num_images_per_control = config["num_images_per_control"]
    if num_images_per_control > 1:
        for path in config["control_paths"]:
            for _ in range(num_images_per_control):
                control_paths.append(path)
        config["control_paths"] = control_paths
    if not args.model_types:
        print("No model types provided, default use all.")
        models_to_run = ["base", "canny", "palette"]
    else:
        models_to_run = args.model_types
    for model_type in models_to_run:
        if model_type not in ["base", "canny", "palette"]:
        ...
        pipeline = load_model("controlnet", config[model_key], config['base_model_path'])
