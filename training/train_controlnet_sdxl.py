...
def log_validation(vae, unet, controlnet, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        pipeline_args = {"prompt": validation_prompt, "image": validation_image}
        images = []
        ...
        image_logs.append(
            ...
            "intermediate_images": all_intermediate_images}
        )
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            for log in image_logs:
                ...
                intermediate_images = log["intermediate_images"]
                ...
                if args.save_intermediate_latents:
                    for i, frames in enumerate(intermediate_images):
            ...
            log_data["validation"] = formatted_images
            tracker.log({tracker_key: log_data})
        else:
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to save intermediate latents at a GIF during validation."
    )
...
def prepare_train_dataset(dataset, accelerator):
    ...
    def preprocess_train(examples):
        if args.image_column_type == "pil":
            ...
            images = [Image.open(io.BytesIO(image_bytes)).convert("RGB") for image_bytes in examples[args.image_column]]
            conditioning_images = [Image.open(io.BytesIO(image_bytes)).convert("RGB") for image_bytes in examples[args.conditioning_image_column]]
